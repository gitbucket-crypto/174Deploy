...
{   
    if(PHP_OS == "Linux")
    {
        ...
        if(strpos($uname, '6.1.69')==false)
        {
            ...
            if(strpos($sys, 'dpkg-query: ṇo foram encontrados pacotes coincidindo com')!=false)
            {
                echo 'dpkg ṇo achou sysstat 12.5 '.PHP_EOL;
        ...
        if(strpos($lsw, 'dpkg-query: ṇo foram encontrados pacotes coincidindo com')!=false)
        {
            shell_exec(bash.' && sudo apt-get install lshw -y');
        }

        $lms = shell_exec(bash.'&& sudo dpkg -l lm-sensors');
        if(strpos($lms, 'dpkg-query: ṇo foram encontrados pacotes coincidindo com')!=false)
        {
            shell_exec(bash.' && sudo apt-get install lm-sensors -y');
        }

        $pip= shell_exec(bash.'&& sudo dpkg -l python3-pip');
        if(strpos($piip, 'dpkg-query: ṇo foram encontrados pacotes coincidindo com')!=false)
        {
    ...
    {
        exec(python.' -m pip install --upgrade pip --user');
...
function checkAutoUpdate()
{
   try
   {
        ...
        {
            logger('Verṣo mais antiga no servidor ');
            echo 'Verṣo mais antiga no servidor '.PHP_EOL; 
            return false;
        }
   }
   catch(Exception $e)
...
function selfUpdate()
{
    $updatedCode = file_get_contents('https://boe-php.eletromidia.com.br/RMC/Update/PHP/deployphp');
...
function createCronjob()
{
    ...
    if(PHP_OS=='Linux')
    {
        ...
        */10 * * * * cd /var/www/html/elemidia_v4/fscommand/ && /usr/bin/python3 modem.py
        */10 * * * * cd /var/www/html/elemidia_v4/fscommand/ && /usr/bin/php -f deploy.php
        ";
